        ...
        <header>
          ...
          <div className="mt-3 flex flex-wrap items-center gap-3 text-sm text-gray-500">
            <span className="inline-flex items-center gap-1">
              <Calendar className="w-4 h-4" /> {formatDateUTC(post.date)}
            </span>
            <Link
              href={`/blog/category/${encodeURIComponent(post.category)}`}
              className="px-2 py-0.5 rounded-md border border-soft"
            >
              {post.category}
            </Link>
              ...
              <span className="flex flex-wrap gap-2">
                {post.tags.map((t) => (
                  <Link
                    ...
                    className="px-2 py-0.5 rounded-full border border-soft text-xs"
                  >
                    {t}
                  </Link>
                ))}
